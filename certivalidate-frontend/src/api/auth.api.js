...
export const authApi = {
  ...
  register: async ({ nombre, apellido, email, password }) => {
    const res = await request('/auth/register', { method: 'POST', body: { nombre, apellido, email, password }, auth: false });
    return res.data;
  },
  ...
  changePassword: async (currentPassword, newPassword) => {
    const res = await request('/auth/perfil/password', { method: 'PUT', body: { currentPassword, newPassword } });
    return res.data;
  },

  verificarEmail: async (token) => {
    const res = await request(`/auth/verificar-email?token=${encodeURIComponent(token)}`, { auth: false });
    return res.data;
  },
  ...
  enable2FA: async (code) => {
    const res = await request('/auth/2fa/enable', { method: 'POST', body: { code } });
    return res.data;
  },

  disable2FA: async (password, code) => {
    const res = await request('/auth/2fa/disable', { method: 'POST', body: { password, code } });
    return res.data;
  },

  verify2FA: async (partial_token, code) => {
    const res = await request('/auth/2fa/verify', { method: 'POST', body: { partial_token, code }, auth: false });
